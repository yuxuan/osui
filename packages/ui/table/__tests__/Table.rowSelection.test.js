...
describe('Table.rowSelection', () => {
  ...
  it('should keep all checked state when remove item from dataSource', () => {
    ...
    expect(wrapper.find('.ant-checkbox').length).toBe(5);
    expect(wrapper.find('.ant-checkbox-checked').length).toBe(5);
    ...
    expect(wrapper.find('.ant-checkbox').length).toBe(4);
    expect(wrapper.find('.ant-checkbox-checked').length).toBe(4);
  });
  ...
  it.skip('select by checkbox to trigger stopPropagation', () => {
    const wrapper = mount(createTable());
    expect(() => {
