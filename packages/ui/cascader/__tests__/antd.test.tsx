...
describe('Cascader', () => {
  ...
  it('backspace should work with `Cascader[showSearch]`', () => {
    ...
    expect(wrapper.find('Cascader').at(0).state('inputValue')).toBe('123');
  ...
  it('should highlight keyword and filter when search in Cascader', () => {
    ...
    expect(wrapper.find('Cascader').at(0).state('inputValue')).toBe('z');
  ...
  it('should render not found content', () => {
    ...
    expect(wrapper.find('Cascader').at(0).state('inputValue')).toBe('__notfoundkeyword__');
    const popupWrapper = mount(wrapper.find('Trigger').instance().getComponent());
    expect(popupWrapper.render()).toMatchSnapshot();
  });

  it('should support to clear selection', async () => {
    const wrapper = mount(<Cascader displayRender={displayRender} options={options} defaultValue={['zhejiang', 'hangzhou']} />);
    const willUnmount = jest.spyOn(wrapper.find('Cascader').at(0).instance(), 'componentWillUnmount');
  ...
  it('should clear search input when clear selection', () => {
    ...
    expect(wrapper.find('Cascader').at(0).state('inputValue')).toBe('xxx');
    wrapper.find('.ant-cascader-picker-clear').at(0).simulate('click');
    expect(wrapper.find('Cascader').at(0).state('inputValue')).toBe('');
  });
  ...
  it('should not trigger visible change when click search input', () => {
    ...
    wrapper.find('Cascader').at(0).setState({ popupVisible: false });
    wrapper.find('input').simulate('click');
  ...
  it('can use fieldNames', () => {
    ...
    wrapper.find('Cascader').at(0).instance().handleChange(['zhejiang', 'hangzhou', 'xihu'], customerOptions);
  ...
  it('defaultValue works correctly when no match options', () => {
    const wrapper = mount(<Cascader displayRender={displayRender} options={options} defaultValue={['options1', 'options2']} />);
  ...
  it('options should open after press esc and then search', () => {
    ...
    wrapper.find('input').simulate('change', { target: { value: 'jin' } });
    expect(wrapper.find('Cascader').at(0).state('popupVisible')).toBe(true);
  });
