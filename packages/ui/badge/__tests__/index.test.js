...
describe('Badge', () => {
    ...
    it('badge should support float number', () => {
        ...
        expect(
            wrapper.find('.ant-badge-multiple-words').first().text()
        ).toEqual('3.5');
    ...
    it('should support offset when count is a ReactNode', () => {
        const wrapper = mount(
            <Badge
                count={<span className="custom" style={{color: '#f5222d'}} />}
                offset={[10, 20]}
            >
                <a href="#" className="head-example">
                    head
    ...
    it('should color style  works on Badge', () => {
        ...
        expect(wrapper.find('.ant-badge-status-text').props().style.color).toBe(
            'red'
        );
    });
...
describe('Ribbon', () => {
    ...
    describe('placement', () => {
        it('works with `start` & `end` placement', () => {
            ...
            expect(
                wrapperStart.find('.ant-ribbon-placement-start').length
            ).toEqual(1);
            ...
            expect(wrapperEnd.find('.ant-ribbon-placement-end').length).toEqual(
                1
            );
        });
    });
    ...
    describe('color', () => {
        ...
        it('works with custom color', () => {
            ...
            expect(
                wrapperLeft.find('.ant-ribbon').prop('style').background
            ).toEqual('#888');
            expect(
                wrapperLeft.find('.ant-ribbon-corner').prop('style').color
            ).toEqual('#888');
            ...
            expect(
                wrapperRight.find('.ant-ribbon').prop('style').background
            ).toEqual('#888');
            expect(
                wrapperRight.find('.ant-ribbon-corner').prop('style').color
            ).toEqual('#888');
        });
    });
