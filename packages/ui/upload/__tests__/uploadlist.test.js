...
const fileList = [
    {
        ...
        thumbUrl:
            'https://zos.alipayobjects.com/rmsportal/IQKRngzUuFzJzGzRJXUs.png',
    },
    {
        uid: '-2',
        name: 'yyy.png',
...
describe('Upload List', () => {
    ...
    it('should use file.thumbUrl for <img /> in priority', () => {
        ...
        fileList.forEach((file, i) => {
            const linkNode = wrapper
                .find('.ant-upload-list-item-thumbnail')
                .at(i);
            const imgNode = wrapper
    ...
    it('should remove correct item when uid is 0', async () => {
        ...
        await act(async () => {
            ...
            const domNode = wrapper
                .find('.ant-upload-list-text-container')
                .at(0)
                .hostNodes()
                .instance();
        ...
        expect(
            wrapper.find('.ant-upload-list-text-container').hostNodes().length
        ).toBe(1);
    ...
    it('In the case of listType=picture, the error status does not show the download.', () => {
        ...
        expect(
            wrapper.find('div.ant-upload-list-item i.anticon-download').length
        ).toBe(0);
    ...
    it('should non-image format file preview', () => {
        const list = [
            ...
            },
            {
                ...
                uid: '-11',
                url:
                    'https://cdn.xxx.com/xx.xx/aaa.png?query=some.query.with.dot',
            },
            {
                name: 'image',
                status: 'done',
    ...
    it('extname should work correctly when url exists', (done) => {
        ...
        wrapper
            .find('div.ant-upload-list-item .anticon-download')
            .simulate('click');
    });
    ...
    it('when picture-card is loading, icon should render correctly', () => {
        ...
        expect(wrapper.find('.ant-upload-list-item-thumbnail').length).toBe(1);
        expect(wrapper.find('.ant-upload-list-item-thumbnail').text()).toBe(
            'uploading'
        );
    ...
    describe('thumbUrl support for non-image', () => {
        ...
        it('should render <img /> when upload non-image file and configure thumbUrl in onChange', async () => {
            ...
            wrapper
                .find('input')
                .simulate('change', {target: {files: [nonImageFile]}});
            ...
            const afterImgNode = wrapper.find(
    ...
    it('itemRender', () => {
        ...
        wrapper
            .find('.custom-item-render-action-remove')
            .first()
            .simulate('click');
        expect(onRemove.mock.calls[0][0]).toEqual(fileList[0]);

        wrapper
            .find('.custom-item-render-action-download')
            .first()
            .simulate('click');
        expect(onDownload.mock.calls[0][0]).toEqual(fileList[0]);

        wrapper
            .find('.custom-item-render-action-preview')
            .first()
            .simulate('click');
